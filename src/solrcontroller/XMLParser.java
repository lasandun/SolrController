...
public class XMLParser {
    ...
    String pathToXML, destinationPath, sourceName;
    int fileCounter;
    
    public XMLParser(String pathToXML, String destinationPath, String sourceName) {
    ...
    public void parseXMLFiles() {
        ...
        for(String fileName : xmlFiles) {
            try {
                ...
                parseFile(fileName, writer, false);
            } catch (Exception ex) {
    ...
    public void parseOneXMLFile() {
        try {
            System.out.println("parsing file : " + pathToXML);
            // empty file name should be given
            parseFile("", null, true);
        } catch (Exception ex) {
            System.out.println("error while parsing file: " + pathToXML);
            Logger.getLogger(XMLParser.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    void parseFile(String fileName, PrintWriter metadataWriter, boolean oneXML) throws Exception {
        ...
        Iterator children = documentElement.getChildElements();
            while(children.hasNext()) {
                ...
                // for use of parseOneXMLFile()
                if(oneXML) {
                    fileName = pathToXML;
                }
                String dateID = "i" + fileName.substring(2, fileName.length()-4).replaceAll("-", "");
                String counter = String.format("%04d", fileCounter++);
                String sourceID = SolrDocUniqueIDHandler.getTextSourceID(sourceName);
                String id = counter + sourceID + dateID;
                idField.setText(id);
        ...
        String filePrefix = rawXMLWithoutExtension;
        String outpath;
        if(oneXML) {
            File f = new File(pathToXML);
            outpath = destinationPath + f.getName();
        } else {
             outpath = destinationPath + filePrefix + ".xml";
        }
